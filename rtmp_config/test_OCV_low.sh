...
rm -f $OUTPUT_DIR/low/low*.ts $OUTPUT_DIR/low/low*.m3u8
...
if [ $? -eq 0 ]; then
    echo "OCV Low-quality streaming started successfully."
else
    echo "Error occurred while starting the OCV low-quality stream."
fi
